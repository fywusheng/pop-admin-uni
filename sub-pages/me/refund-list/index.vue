<template>
  <div class="page-refund-list">
    <ul class="item-list">
      <li class="item" @click="toDetail(item)" :key="itemIndex" v-for="(item,itemIndex) in itemList">
  ...
  export default {
    ...
    methods: {
      async loadData() {
        ...
        const result = await Axios.get('/aftersale/returns/list', {params})
        wx.hideLoading()
        if (result.code == 200) {
      ...
      toDetail(item) {
        wx.navigateTo({
          url: `../refund-detail/main?returnId=${item.id}`
        })
      },
      toItem(item) {
        wx.navigateTo({
