      ...
      <template v-if="status === 0">
        <ul class="record-list border-t">
          <li class="record border-b" v-for="(data,index) in allList" :key="index">
            <div class="record__title">{{data.typeDes}}&nbsp;</div>
            <div class="record__desc">{{data.createdTime}}&nbsp;</div>
            <div class="record__money">{{data.mathType === 1 ? '+':'-'}}{{data.points}}</div>
          </li>
        </ul>
        <div v-if="!allList.length" class="empty">
          <div class="title">暂没有积分</div>
          <div class="desc">购买商品将获得积分</div>
        </div>
      </template>
      <template v-if="status === 1">
        <ul class="record-list border-t">
          <li class="record border-b" v-for="(data,index) in incomeList" :key="index">
            <div class="record__title">{{data.typeDes}}&nbsp;</div>
            <div class="record__desc">{{data.createdTime}}&nbsp;</div>
            <div class="record__money">+{{data.points}}</div>
          </li>
        </ul>
...
import Axios from "@/config/axios.js"
  export default {
    ...
    computed: {
      costList() {
        const list = [];
        this.allList && this.allList.forEach(data => {
          if (data.mathType == -1) {
            list.push(data)
          }
        })
    ...
    methods: {
      ...
      async loadData() {
        wx.showLoading();
        //this.loading = true;
        const result = await Axios.post('/member/points.details', {
        ...
        if (result.code == 200) {
          ...
          wx.showToast(result.msg || '获取积分信息失败！');
        }
      }
    },
    async mounted() {
      wx.showLoading()
      const result = await Axios.get('/member/points.accumulation',{sessionId: Store.state.login.sessionId})
      wx.hideLoading()
      if (result.code == 200) {
        this.balance = result.data.points;
      }
      this.loadData()
    }
  }
