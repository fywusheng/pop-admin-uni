<template>
  <div class="page-address-list" :class="{'select': type === 1}">
    <ul class="delivery-list" v-if="loading || dataList.length">
      <li class="delivery" v-for="(delivery,index) in dataList"
          :key="index">
        <div class="checkbox" v-if="type === 1">
          <img v-if="delivery.id == addressId" src="/static/images/common/select_h.png">
          <img @click="selectAddress(delivery)" v-else src="/static/images/common/select_n.png">
        </div>
        <div class="name-wrap">
        ...
        <div class="btn-edit" v-if="type === 1" @click.stop="toEdit(delivery)"><img src="/static/images/address/icon-edit.png"></div>
        <div class="delivery-fb" v-if="type !== 1" @tap.stop="">
          <img class="checkbox" v-if="delivery.isDefault" src="/static/images/common/select_h.png">
          <img class="checkbox" @click="setDefault(delivery)" v-else src="/static/images/common/select_n.png">
          <div class="desc">默认地址</div>
          <div class="btn-wrap">
            <div class="btn-edit" @click.stop="toEdit(delivery)"><img class="icon-opr" src="/static/images/address/icon-edit.png">编辑</div>
            <div class="btn-edit" @click.stop="toDelete(delivery)"><img class="icon-opr" src="/static/images/address/icon-delete.png">删除</div>
          </div>
        </div>
      </li>
    </ul>
    ...
    <button v-else type="button" class="btn-bottom-fixed" :class="{isIphoneHair}" @click="add">+添加新的收货地址</button>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    async mounted() {
      ...
      this.addressId = this.$root.$mp.query.addressId || State.checkout.addressId;
      if(this.type === 1){
